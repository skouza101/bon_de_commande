...
def fetch_supported_gemini_models(api_key: str) -> List[Dict[str, Any]]:
    ...
    if not supported:
        supported = [
            {"id": "gemini-2.5-flash", "name": "Gemini 2.5 Flash", "description": "Modèle rapide recommandé", "is_flash": True},
            {"id": "gemini-2.0-flash", "name": "Gemini 2.0 Flash", "description": "Modèle ultra rapide", "is_flash": True},
            {"id": "gemini-1.5-flash", "name": "Gemini 1.5 Flash", "description": "Modèle standard", "is_flash": True},
            {"id": "gemini-1.5-pro", "name": "Gemini 1.5 Pro", "description": "Modèle haute précision", "is_flash": False},
        ]
    return supported
    ...
    if not key:
        return JSONResponse(
            content={
                ...
                "provider": "gemini",
                "models": [
                    {"id": "gemini-2.5-flash", "name": "Gemini 2.5 Flash (Par défaut)", "is_flash": True},
                    {"id": "gemini-2.0-flash", "name": "Gemini 2.0 Flash", "is_flash": True},
                    {"id": "gemini-1.5-flash", "name": "Gemini 1.5 Flash", "is_flash": True},
                    {"id": "gemini-1.5-pro", "name": "Gemini 1.5 Pro", "is_flash": False},
                ],
...
async def test_ai_key(payload: Optional[TestKeyRequest] = None):
    ...
    candidate_ids = [m["id"] for m in supported_models]
    if settings.gemini_model in candidate_ids:
        candidate_ids.remove(settings.gemini_model)
        candidate_ids.insert(0, settings.gemini_model)
